// Lifted from EZ Flash Omega Kernel source
...
struct SaveModeEntry {	
	char gamecode[4];		
	uint8_t savemode;	
};

EWRAM_RODATA static const struct SaveModeEntry  __attribute__((aligned(4))) SAVE_MODE_TABLE[] = { 
...
EWRAM_CODE THUMB static bool strcmp_n_chars(const uint8_t * const lhs, const uint8_t * const rhs, const uint32_t n) {
...
enum SaveType lookup_rom_savetype() {
    EWRAM_RODATA static const uint8_t TERMINAL_ID[] = "FFFF";
    const uint8_t * const rom_header_ptr = (const uint8_t *) 0x08000000 + 0xac;

    for (uint32_t i = 0; ; ++i) {
        if (strcmp_n_chars((const uint8_t *) SAVE_MODE_TABLE[i].gamecode, TERMINAL_ID, 4)) {
            return SAV_UNKNOWN;
        }
        if (!strcmp_n_chars((const uint8_t *) SAVE_MODE_TABLE[i].gamecode, rom_header_ptr, 4)) {
            continue;
        }

        switch (SAVE_MODE_TABLE[i].savemode) {
            case 0x00: // No save
                return SAV_NONE;
            case 0x11: // SRAM 32k
                return SAV_SRAM_32k;
            case 0x21: // EEPROM 512
                return SAV_EEPROM_512;
            case 0x22: // EEPROM 8k
                return SAV_EEPROM_8k;
            case 0x23: // EEPROM 8k v125 v126
                return SAV_EEPROM_8k_v125_v126;
            case 0x31: // FLASH1M 128k
                return SAV_FLASH1M_128k;
            case 0x32: // FLASH 64k
                return SAV_FLASH_64k;
            case 0x33: // FLASH512 128k
                return SAV_FLASH512_128k;
            default:
                m3_log_inline("Unknown entry in game db");
                panic();
                break;
        }
        break;
